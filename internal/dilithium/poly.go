...
func polyUniform(seed [SeedSize]byte, nonce uint16) Poly {
	var a Poly
...
func polyUniformEta(seed [2 * SeedSize]byte, nonce uint16, ETA int32) Poly {
	var a Poly
	blocks := 1 //ETA == 2
	if ETA == 4 {
		blocks = 2
...
func polyUniformGamma1(rhoP [2 * SeedSize]byte, nonce uint16, GAMMA1 int32) Poly {
